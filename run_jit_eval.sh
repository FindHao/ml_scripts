...
output=/home/yhao/d/tmp/run_jit_eval.log
echo "" > $output
cd /home/yhao/d/benchmark_11
max_iter=20
func(){
    for (( i = 1 ; i <= $max_iter; i++ ))
    do
        python run.py -d cuda -t eval  $model --precision fp32  >> $output 2>&1
    done
}
func2(){
    for (( i = 1 ; i <= $max_iter; i++ ))
...
# for model in squeezenet1_1 timm_efficientnet shufflenet_v2_x1_0 resnext opacus_cifar10 detectron2_fasterrcnn_r_50_fpn detectron2_maskrcnn
do 
...
func
echo "@Yueming Hao jit $model" >>$output
func
