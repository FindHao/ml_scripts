...
for model in Super_SloMo pytorch_struct pplbench_beanmachine drq pyhpc_isoneutral_mixing hf_Albert attention_is_all_you_need_pytorch moco Background_Matting pyhpc_turbulent_kinetic_energy maml_omniglot pyhpc_equation_of_state timm_nfnet demucs densenet121 pytorch_CycleGAN_and_pix2pix tacotron2 squeezenet1_1 fastNLP_Bert pytorch_stargan hf_DistilBert speech_transformer yolov3 timm_efficientdet
do 
...
python run.py -d cuda --flops dcgm $model  >> $output 2>&1
echo "@Yueming Hao: end model tests" >> $output

done

cd /home/yhao/d/testing/
python3 filter.py 