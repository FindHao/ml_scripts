...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        event_buf0_buf1 = torch.cuda.Event()
        ...
        event_buf0_buf1.record(stream0_raw)
        del arg13_1
        ...
        stream1_raw.wait_event(event_buf0_buf1)
        event_buf3 = torch.cuda.Event()
        torch.cuda.set_stream(stream1_raw)
        extern_kernels.mm(as_strided(buf1, (8192, 512), (512, 1)), as_strided(arg71_1, (512, 512), (1, 512)), out=buf3)
        torch.cuda.set_stream(stream0_raw)
        event_buf3.record(stream1_raw)
        del arg71_1
        ...
        del buf2
        torch.cuda.set_stream(stream3_raw)
        buf5 = empty_strided((4, 8, 64, 2048), (1048576, 131072, 2048, 1), device='cuda', dtype=torch.bfloat16)
        torch.cuda.set_stream(stream0_raw)
        stream3_raw.wait_event(event_buf3)
        event_buf5 = torch.cuda.Event()
        torch.cuda.set_stream(stream3_raw)
        triton_poi_fused_clone_2.run(buf3, buf5, 2048, 2048, grid=grid(2048, 2048), stream=stream3)
        torch.cuda.set_stream(stream0_raw)
        event_buf5.record(stream3_raw)
        buf6 = empty_strided((32, 2048, 2048), (4194304, 2048, 1), device='cuda', dtype=torch.bfloat16)
        stream0_raw.wait_event(event_buf5)
        ...
        del buf7
        torch.cuda.set_stream(stream2_raw)
        buf10 = empty_strided((8192, 512), (512, 1), device='cuda', dtype=torch.bfloat16)
        torch.cuda.set_stream(stream0_raw)
        stream2_raw.wait_event(event_buf0_buf1)
        event_buf10 = torch.cuda.Event()
        torch.cuda.set_stream(stream2_raw)
        extern_kernels.mm(as_strided(buf1, (8192, 512), (512, 1)), as_strided(arg72_1, (512, 512), (1, 512)), out=buf10)
        torch.cuda.set_stream(stream0_raw)
        event_buf10.record(stream2_raw)
        del arg72_1
        del buf1
        torch.cuda.set_stream(stream4_raw)
        buf12 = empty_strided((4, 8, 2048, 64), (1048576, 131072, 64, 1), device='cuda', dtype=torch.bfloat16)
        torch.cuda.set_stream(stream0_raw)
        stream4_raw.wait_event(event_buf10)
        event_buf12 = torch.cuda.Event()
        torch.cuda.set_stream(stream4_raw)
        triton_poi_fused_clone_1.run(buf10, buf12, 4194304, grid=grid(4194304), stream=stream4)
        torch.cuda.set_stream(stream0_raw)
        event_buf12.record(stream4_raw)
        buf13 = empty_strided((32, 2048, 64), (131072, 64, 1), device='cuda', dtype=torch.bfloat16)
        stream0_raw.wait_event(event_buf12)
        ...
        event_buf18.record(stream0_raw)
        # torch.cuda.set_stream(stream0_raw)
        del arg75_1
        del buf17
        ...
        stream5_raw.wait_event(event_buf18)
        torch.cuda.set_stream(stream5_raw)
        buf151 = empty_strided((4, 8, 2048, 64), (1048576, 131072, 64, 1), device='cuda', dtype=torch.bfloat16)
        torch.cuda.set_stream(stream0_raw)
        event_buf151 = torch.cuda.Event()
        torch.cuda.set_stream(stream5_raw)
        triton_poi_fused_clone_1.run(buf18, buf151, 4194304, grid=grid(4194304), stream=stream5)
        torch.cuda.set_stream(stream0_raw)
        event_buf151.record(stream5_raw)
        del buf18
