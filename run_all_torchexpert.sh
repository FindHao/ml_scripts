...
func(){
    ...
    do
        ...
        python run.py -d cuda -t eval --torchexpert --profile-detailed --profile-devices cpu,cuda --profile-folder /tmp/logs_profile/$model  $model  --precision fp32  >> $output 2>&1
        if [ $? -ne 0 ]; then
            break
        fi
    done
